            ...
            LinkSubcommand::Claims(args) => {
                let path_prefix = args
                    ...
                    .map(|p| {
                        ...
                        Ok(normalize_claim_path(trimmed))
                    })
                    .transpose()?;
...
/// Normalize a claim path: strip `./` prefix, trailing `/`, collapse `.` and `..`.
pub(crate) fn normalize_claim_path(s: &str) -> String {
...
mod tests {
    ...
    #[test]
    fn normalize_claim_path_strips_dot_slash() {
        assert_eq!(normalize_claim_path("./src/foo.rs"), "src/foo.rs");
    }

    #[test]
    fn normalize_claim_path_strips_trailing_slash() {
        assert_eq!(normalize_claim_path("src/"), "src");
    }

    #[test]
    fn normalize_claim_path_collapses_dotdot() {
        assert_eq!(normalize_claim_path("src/lib/../foo.rs"), "src/foo.rs");
    }

    #[test]
    fn normalize_claim_path_identity() {
        assert_eq!(normalize_claim_path("src/foo.rs"), "src/foo.rs");
    }
