...
pub(crate) fn run(platform: crate::cli::Platform, current_dir: &Path) -> anyhow::Result<()> {
    ...
    match cmd {
        Cmd::Claim { paths, ttl } => handle_claim_batch(&conn, &agent_id, &paths, ttl),
        Cmd::Release { paths } => handle_release_batch(&conn, &agent_id, &paths),
...
fn handle_claim_batch(
    ...
    let mut delete_stmt = conn.prepare("DELETE FROM claims WHERE path = ?1 AND agent_id = ?2")?;
    let mut insert_stmt =
        conn.prepare("INSERT INTO claims(path, agent_id, expires_at_ms) VALUES (?1, ?2, ?3)")?;
...
fn handle_release_batch(conn: &Connection, agent_id: &str, paths: &[String]) -> anyhow::Result<()> {
    let mut delete_stmt = conn.prepare("DELETE FROM claims WHERE path = ?1 AND agent_id = ?2")?;
    let normalized = normalized_unique_paths(paths);
    for path in &normalized {
        delete_stmt.execute(params![path, agent_id])?;
        ...
        conn.prepare(
            "SELECT path, agent_id, expires_at_ms FROM claims \
             WHERE expires_at_ms > ?1 \
               AND (path = ?2 OR ?2 GLOB path || '/*' OR path GLOB ?2 || '/*') \
        ...
        .query_row(
            "SELECT path, expires_at_ms FROM claims \
             WHERE agent_id = ?1 AND expires_at_ms > ?2 \
               AND (path = ?3 OR ?3 GLOB path || '/*' OR path GLOB ?3 || '/*') \
    ...
    let mut stmt = conn.prepare(
        "SELECT agent_id, path, expires_at_ms FROM claims \
         WHERE agent_id <> ?2 AND expires_at_ms > ?3 \
           AND (path = ?1 OR ?1 GLOB path || '/*' OR path GLOB ?1 || '/*') \
...
fn handle_discovery(
    ...
    if let Some(sig) = signal {
        payload
            .as_object_mut()
            .unwrap()
            .insert("signal".to_owned(), Value::String(sig.to_owned()));
    }
...
fn handle_read(conn: &Connection, kind: Option<String>, since: Option<i64>) -> anyhow::Result<()> {
    ...
    if kind == "discovery" || kind == "all" {
        ...
        kind.as_str(),
        "declaration" | "intent" | "message" | "block"
    ) {
        let mut stmt =
            conn.prepare("SELECT created_at_ms, agent_id, body_json FROM messages WHERE kind = ?1 ORDER BY id ASC")?;
