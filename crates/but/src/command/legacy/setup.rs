...
fn hook_results_to_hook_info(
    results: &[(String, gitbutler_repo::managed_hooks::HookBackupStatus)],
) -> Vec<HookInfo> {
    results
        .iter()
...
pub(crate) fn repo(
    ...
    if no_hooks {
        ...
        match outcome {
            gitbutler_repo::managed_hooks::HookSetupOutcome::Installed { ref hook_results } => {
            ...
            gitbutler_repo::managed_hooks::HookSetupOutcome::AlreadyInstalled => {
                ...
                if let Some(out) = out.for_human() {
                    writeln!(
                        out,
                        "  {}",
                        "✓ Hooks already configured (pre-commit, post-checkout, pre-push)".green()
                    )?;
                }
            ...
            gitbutler_repo::managed_hooks::HookSetupOutcome::PartialSuccess {
                ...
                hook_info_json.extend(but_hooks::hook_manager::MANAGED_HOOK_NAMES.iter().map(
                    |name| HookInfo {
                        name: name.to_string(),
                        status: if installed_hooks.contains(&name.to_string()) {
                            HookStatus::Installed { backup_path: None }
                        } else {
                            HookStatus::Skipped
                        },
                    },
            ...
            gitbutler_repo::managed_hooks::HookSetupOutcome::HookSkipped { ref hook_names } => {
                if let Some(out) = out.for_human() {
                    writeln!(
                        out,
                        "  {}",
                ...
                && let Some(out) = out.for_human()
            {
                ...
                writeln!(
                    ...
                    format!(
                        ...
                        gitbutler_repo::hook_manager::orphaned_hooks_remove_command()
                    )
                    .dimmed()
                )?;
            }
