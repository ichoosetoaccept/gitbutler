...
fn git_checkout_from_workspace_removes_managed_hooks() -> anyhow::Result<()> {
    ...
    for hook_name in ["pre-commit", "post-checkout", "pre-push"] {
        assert!(
            ...
            "{hook_name} hook should be installed after setup"
        );
    }
    ...
    for hook_name in ["pre-commit", "post-checkout", "pre-push"] {
        assert!(
            !hooks_dir.join(hook_name).exists(),
            "{hook_name} should be removed after leaving workspace"
        );
    }
...
fn setup_teardown_hook_lifecycle() -> anyhow::Result<()> {
    ...
    for hook_name in ["pre-commit", "post-checkout", "pre-push"] {
        ...
        assert!(hook_path.exists(), "{hook_name} should exist after setup");
...
"#]]);
    ...
    for hook_name in ["pre-commit", "post-checkout", "pre-push"] {
        assert!(
            !hooks_dir.join(hook_name).exists(),
            "{hook_name} should be removed after teardown"
        );
    }
