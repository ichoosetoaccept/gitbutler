...
# Not on workspace branch - warn if a backup hook exists but we are not on workspace.
# (The user hook was already run above, before the workspace check.)
if [ -x "$HOOKS_DIR/pre-commit-user" ]; then
    echo ""
...
pub enum HookBackupStatus {
    ...
    Created(String),
    /// A backup already existed at the indicated path (no new backup created).
    Existed(String),
}

impl HookBackupStatus {
    /// Returns the backup path if one is associated with this status, or `None`.
    ///
    /// Both [`HookBackupStatus::Created`] and [`HookBackupStatus::Existed`] carry a
    /// path; [`HookBackupStatus::None`] does not.
    pub fn to_backup_path(&self) -> Option<&str> {
        match self {
            Self::None => Option::None,
            Self::Created(p) | Self::Existed(p) => Some(p.as_str()),
        }
    }
    ...
    /// - `"✓ Installed pre-commit (backup already existed: pre-commit-user)"`
    pub fn format_install_line(&self, name: &str) -> String {
        match self {
            ...
            Self::Existed(backup) => {
                format!("✓ Installed {name} (backup already existed: {backup})")
            }
        }
    }
}
    ...
    if hook_path.exists() && is_gitbutler_managed_hook(&hook_path) {
        ...
        if force {
            if !user_backup_path.exists() {
                ...
                backup_status = HookBackupStatus::Existed(hook_type.user_backup_name().to_string());
            }
        } else {
            tracing::info!(
...
pub fn install_managed_hooks(hooks_dir: &Path, force: bool) -> Result<HookInstallationResult> {
    ...
    let mut hook_results: Vec<(String, HookBackupStatus)> = Vec::new();

    for hook_type in MANAGED_HOOK_TYPES {
        match install_hook(hooks_dir, hook_type, force) {
            Ok(HookInstallationResult::Success { hook_results: hr }) => {
                tracing::debug!("Installed {} hook", hook_type.hook_name());
                success_count += 1;
                installed_hooks.push(hook_type.hook_name().to_owned());
                hook_results.extend(hr);
            }
    ...
    if !skipped_hooks.is_empty() {
        if success_count > 0 || already_configured_count > 0 {
            warnings.extend(
                skipped_hooks.iter().map(|name| {
                    format!(
                        "Skipped {name} — hook exists and is not GitButler-managed — use --force-hooks to override"
                    )
                }),
            );
        ...
        return Ok(HookInstallationResult::Skipped {
            hook_names: skipped_hooks,
        });
    }

    if already_configured_count == MANAGED_HOOK_TYPES.len() && warnings.is_empty() {
        Ok(HookInstallationResult::AlreadyConfigured)
