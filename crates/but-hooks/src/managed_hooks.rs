...
if echo "$PREV_BRANCH" | grep -qE '(^|/)gitbutler/workspace($|[~^])'; then
...
pub fn install_managed_hooks(hooks_dir: &Path, force: bool) -> Result<HookInstallationResult> {
    ...
    // If any hooks were skipped, decide based on whether other hooks succeeded.
    if !skipped_hooks.is_empty() {
        if success_count > 0 || already_configured_count > 0 {
            // Mixed result: some installed, some skipped — report as partial success
            // so callers know hooks *are* active on disk.
            warnings.extend(
                skipped_hooks.iter().map(|name| {
                    format!("Skipped {name} — hook exists and is not GitButler-managed")
                }),
            );
            return Ok(HookInstallationResult::PartialSuccess { warnings });
        }
        return Ok(HookInstallationResult::Skipped {
...
pub enum HookSetupOutcome {
    ...
    Installed,
    /// Most hooks were installed but one or more individual hooks failed.
    /// The warnings describe which hooks failed and why. Callers should treat
    /// this as a partial success — hooks are partially active — and surface
    /// the warnings to the user.
    PartialSuccess {
        /// Human-readable descriptions of each hook that failed to install.
        warnings: Vec<String>,
    },
...
pub fn ensure_managed_hooks(
    ...
    match install_managed_hooks(hooks_dir, force) {
        Ok(HookInstallationResult::Success | HookInstallationResult::AlreadyConfigured) => {
            HookSetupOutcome::Installed
        }
        Ok(HookInstallationResult::PartialSuccess { warnings }) => {
            HookSetupOutcome::PartialSuccess { warnings }
        }
        Ok(HookInstallationResult::Skipped { hook_names }) => {
