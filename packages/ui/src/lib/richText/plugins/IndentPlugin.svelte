...
<script lang="ts" module>
	 ...
	 * to the end of the previous sibling so the split moves the node whole.
	 */
	function normalizeNonSimpleTextNode(
		...
		if (prev && isTextNode(prev)) {
			return { textNode: prev, offset: prev.getTextContent().length };
		}
	...
	export function handleEnter(event: KeyboardEvent | null) {
		...
		if (event?.shiftKey) {
			...
			const normalized = normalizeNonSimpleTextNode(rawNode, offset);
